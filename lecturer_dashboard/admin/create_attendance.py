...
class CreateAttendanceCustomAdminsite(admin.ModelAdmin):
    list_display = ['lecturer', 'code', 'subject', 'department', 'levels','status','time_elapsed']
    # list_display_links = ['*']
    list_form = '__all__'
    exclude = ['lecturer','time_elapsed2','attributes','total_number_students']
    m = MODEL._meta
    ...
    def response_add(self, request: HttpRequest, obj, post_url_continue=None) -> HttpResponse:
        if not request.user.is_superuser:
            obj.code =  f"#{uuid.uuid4().hex}"[:5]
            obj.lecturer_id =  request.user.id
            obj.attributes = ''
            obj.total_number_students = 0
            obj.save()
    ...
    def get_queryset(self, request: HttpRequest) -> QuerySet[Any]:
        ...
        if request.user.is_superuser:
            return qs
        return qs.filter(lecturer_id=request.user.id)
...
lecturer_dashboard_site.register(MODEL, CreateAttendanceCustomAdminsite)
